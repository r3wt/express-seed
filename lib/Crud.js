...
class Crud extends EventEmitter {
    ...
    query(req,res) {
        ...
        this.emit('query',req,res,()=>{
            var validate = req.validate('POST',[],['page','pageSize','query','select']);
            ...
            var select = validate.data.select || undefined;
            ...
            Promise.all([
                this.model.find(query).skip(offset).limit(size).select(select).exec(),
